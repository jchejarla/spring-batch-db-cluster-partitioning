...
**Does this work with Spring Batch 6's `ResourcelessJobRepository`? Do I need a JDBC job repository?**
Every node must share one persistent, JDBC-backed `JobRepository`; the resourceless repository Spring Batch 6 defaults to will not work — and that isn't a limitation unique to this extension. Spring Batch's own documentation scopes resourceless to single-JVM jobs and *explicitly excludes* "partitioned steps where partitions meta-data is shared between the manager and workers through the execution context." Distributed partitioning is exactly that case: a partition's slice of work travels in the execution context from the master to the node that runs it and back again to be aggregated, so the metadata store must be shared and durable. Spring Batch's *own* remote partitioning requires the same shared repository — the broker only carries a pointer; the partition's data lives in the repository. So the boundary is Spring Batch's own: a throwaway single-JVM job needs no repository; a partitioned or distributed job needs a shared persistent one. This extension sits on that boundary and asks for nothing beyond the shared database — no broker on top. Enable it with `@EnableBatchProcessing` + `@EnableJdbcJobRepository`.
